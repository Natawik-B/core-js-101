...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${start}${Math.min(a, b)}, ${Math.max(a, b)}${finish}`;
}
...
function isBracketsBalanced(str) {
  ...
  let flag = 0;
  while (flag < brackets.length) {
    flag = 0;
    for (let i = 0; i < brackets.length; i += 1) {
      if (string.includes(brackets[i])) {
        string = string.replaceAll(brackets[i], '');
      } else {
        flag += 1;
      }
    }
...
function getCommonDirectoryPath(pathes) {
  let string = '';
  let i = 0;
  const startsWithPath = () => pathes.every((path) => path.startsWith(string));
  while (startsWithPath()) {
    string = pathes[0].slice(0, i);
    i += 1;
  }
  string = string.slice(0, -1);
  string.lastIndexOf('/');
  return string.length === 0 ? '' : string.slice(0, string.lastIndexOf('/') + 1);
}
